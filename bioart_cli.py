...
Examples:
  ...
  bioart_cli.py info --dna "AUCGAUCG"
    ...
    encode_parser.add_argument(
        "--format", action="store_true", help="Format output for readability"
    )
    ...
    modify_group.add_argument(
        "--replace",
        nargs=3,
        metavar=("START", "LENGTH", "NEW_SEQ"),
        help="Replace sequence at position",
    )
    modify_group.add_argument(
        "--insert", nargs=2, metavar=("POSITION", "SEQ"), help="Insert sequence at position"
    )
    modify_group.add_argument(
        "--delete", nargs=2, metavar=("START", "LENGTH"), help="Delete sequence"
    )
    modify_group.add_argument(
        "--mutate", nargs=2, metavar=("POSITION", "NUCLEOTIDE"), help="Mutate single nucleotide"
    )
        ...
        try:
            ...
            if cmd == "quit" or cmd == "exit":
                ...
                if arg:
                    ...
                    print(
                        f"Original: {result['original_size']} bytes → DNA: {result['dna_size']} nt → Restored: {result['restored_size']} bytes"
                    )
                else:
