...
class TestCircuitBreaker:
    ...
    def test_init_custom_exceptions(self):
        ...
        cb = CircuitBreaker(
            name="test", failure_threshold=3, expected_exceptions=(ValueError, TypeError)
        )
    ...
    def test_init_none_exceptions_becomes_exception_tuple(self):
        ...
        cb = CircuitBreaker(name="test", failure_threshold=3, expected_exceptions=None)
        assert cb.expected_exceptions == (Exception,)
    ...
    def test_circuit_half_open_after_timeout(self):
        ...
        cb = CircuitBreaker(
            name="test", failure_threshold=2, recovery_timeout=0.1  # Short timeout for testing
        )
    ...
    def test_custom_exceptions_only(self):
        ...
        cb = CircuitBreaker(name="test", failure_threshold=3, expected_exceptions=(ValueError,))
...
class TestCircuitBreakerRegistry:
    ...
    def test_decorator_with_exceptions(self):
        ...
        @circuit_breaker(
            name="test_decorator_exc", failure_threshold=2, expected_exceptions=(ValueError,)
        )
        def failing_func():
            nonlocal call_count
            call_count += 1
