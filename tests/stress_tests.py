...
def test_dna_sequence_patterns():
    ...
    for name, pattern in patterns.items():
        try:
            ...
            print(
                f"   {name:15} {pattern} → {byte_val:3d} → {restored_dna} {'✓' if pattern == restored_dna else '✗'}"
            )
...
def run_stress_tests():
    ...
    try:
        ...
        print(
            f"{'✅' if byte_test_passed else '❌'} All Byte Patterns - {'PASSED' if byte_test_passed else 'FAILED'}"
        )
