...
class DistributedDNAComputer:
    ...
    def __init__(self, node_type: NodeType = NodeType.COORDINATOR, 
                 listen_port: int = 8080, node_id: Optional[str] = None):
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        active_nodes = sum(1 for node in self.compute_nodes.values() 
                          if time.time() - node.last_heartbeat < 60.0)
        ...
        return {
            ...
            'system_load': sum(node.current_load for node in self.compute_nodes.values()) / max(1, len(self.compute_nodes)),
        ...
        while task_id in self.distributed_tasks:
            ...
            if task.status in [TaskStatus.COMPLETED, TaskStatus.FAILED]:
                return {
                    ...
                    'result': task.result,
                    'execution_time': (task.completed_time - task.started_time) if task.started_time and task.completed_time else 0
                }
