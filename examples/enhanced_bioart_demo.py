...
def main():
    ...
    print(
        f"   ✅ System initialized with {capabilities['core_features']['instruction_count']} instructions"
    )
    ...
    try:
        ...
        storage_id = system.store_in_biological_storage(
            test_data, metadata={"type": "demo", "created": time.time()}
        )
        ...
        if retrieved_data:
            ...
            print(
                f"   ✅ Data integrity: {'✅ OK' if retrieved_data == test_data else '❌ CORRUPTED'}"
            )
        else:
    ...
    try:
        ...
        print(
            f"   ✅ Error correction applied: {len(encoded_sequence)} nucleotides (from {len(original_sequence)})"
        )
    ...
    try:
        ...
        mod_id = system.design_crispr_modification(
            target_sequence, "insertion", "AAAA"  # Insert 4 A's
        )
