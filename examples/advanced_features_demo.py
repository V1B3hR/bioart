...
def demo_quantum_error_correction():
    ...
    print(
        f"Quantum state: {quantum_state.num_qubits} qubits, {len(quantum_state.amplitudes)} amplitudes"
    )
...
def demo_workflow_automation():
    ...
    workflow_id = orchestrator.create_workflow_from_template(
        "standard_synthesis", "Demo DNA Synthesis Workflow"
    )
...
def demo_realtime_monitoring():
    ...
    for cycle in range(1, 6):
        ...
        print(
            f"  Cycle {cycle}: {job_status['progress_percentage']:.1f}% complete, Phase: {phase.value}"
        )
...
def demo_integration_pipeline():
    ...
    print(f"  Encoded sequence: {encoded_sequence}")
    print(
        f"  QEC overhead: {len(encoded_sequence) - len(ml_result.optimized_sequence)} nucleotides"
    )
    ...
    print(
        f"  QEC protection: {len(encoded_sequence) - len(ml_result.optimized_sequence)} extra nucleotides"
    )
